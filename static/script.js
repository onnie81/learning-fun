...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const COLOR_F2_FILL_RAW = tailwind.config.theme.extend.colors.fractionTwo; // For borders, etc.
    const COLOR_F2_FILL_TRANSPARENT = tailwind.config.theme.extend.colors.fractionTwoFill;
    const COLOR_F2_EMPTY = 'rgba(255, 215, 0, 0.3)'; // More transparent gold
    ...
    function drawPizza(num1, den1, num2, den2) {
        ...
        // Draw Fraction 1 Pizza
        if (den1 > 0) {
            const anglePerSlice1 = 360 / den1;
            for (let i = 0; i < den1; i++) {
                const path = document.createElementNS('http://www.w3.org/2000/svg', 'path');
                path.setAttribute('d', getPizzaSlicePath(i * anglePerSlice1, (i + 1) * anglePerSlice1));
        ...
        // Draw Fraction 2 Pizza (superimposed)
        if (den2 > 0 && num2 !== null) {
            const anglePerSlice2 = 360 / den2;
            for (let i = 0; i < den2; i++) {
                const path = document.createElementNS('http://www.w3.org/2000/svg', 'path');
                // Pack F2 to the right: slices from (den2 - num2) to den2-1 are filled
                // For drawing, we iterate i from 0. A slice `i` is filled if `i >= den2 - num2`.
                // Or, simpler: fill `num2` slices starting from the "end" of the circle.
                // Let's fill the *last* num2 slices.
                // Slice i is filled if (den2 - 1 - i) < num2
                const isFilled = (den2 - 1 - i) < num2;
                
                // To make it pack from right visually (counter-clockwise from 360 deg point):
                // Start angle for i-th slice from right: 360 - (i+1)*anglePerSlice2
                // End angle for i-th slice from right: 360 - i*anglePerSlice2
                const startAngle = 360 - ((i + 1) * anglePerSlice2);
                const endAngle = 360 - (i * anglePerSlice2);

                path.setAttribute('d', getPizzaSlicePath(startAngle, endAngle));
                path.classList.add('pizza-slice-f2'); // General F2 styling (dotted, base opacity)
                
                if (i < num2) { // Fill the first `num2` slices when counting from the right.
    ...
    function drawRectangle(num1, den1, num2, den2) {
        ...
        // Create separate grid containers for F1 and F2 to allow independent segmentation
        ...
        if (den1 > 0) {
            createRectangleGrid(gridF1Container, num1, den1, 'f1', false); // false for isFractionTwo
        }
        ...
        if (den2 > 0 && num2 !== null) {
            ...
            createRectangleGrid(gridF2Container, num2, den2, 'f2', true); // true for isFractionTwo
        }
    }

    function createRectangleGrid(container, num, den, fractionClassSuffix, isFractionTwo) {
        const numRows = (den % 2 === 0 && den > 1) ? 2 : 1;
        const segmentsPerRow = den / numRows;
        container.style.height = numRows === 1 ? 'calc(50% - 5px)' : '100%'; // Adjust if only one grid, or manage stacking
        if (rectangleContainer.children.length === 2 && numRows === 1) { // If two single-row grids, stack them
             container.style.height = 'calc(50% - 2.5px)';
             if (isFractionTwo) container.style.marginTop = '5px';
        } else if (rectangleContainer.children.length === 1) {
        ...
        for (let r = 0; r < numRows; r++) {
            ...
            for (let c = 0; c < segmentsPerRow; c++) {
                ...
                if (isFractionTwo) segment.classList.add(`rectangle-segment-f2`);
                // segment.textContent = `R${r}C${c}`; // Debug
        ...
        // Fill segments with column-first logic
        for (let k = 0; k < num; k++) {
            let targetSegmentIndex;
            if (!isFractionTwo) { // Fraction 1: pack left
                const colToFill = Math.floor(k / numRows);
                const rowToFill = k % numRows;
                targetSegmentIndex = rowToFill * segmentsPerRow + colToFill;
            } else { // Fraction 2: pack right
                const kFromRight = k; // Filling k-th segment from the right end
                const effectiveTotalIndexFromRight = kFromRight; // 0th from right, 1st from right...
                const colToFill = Math.floor(effectiveTotalIndexFromRight / numRows); // Column from right
                const rowToFill = effectiveTotalIndexFromRight % numRows; // Row in that right-most column
                
                // Convert column-from-right to column-from-left
                const actualColFromLeft = (segmentsPerRow - 1) - colToFill;
    ...
    function drawNumberLine(num1, den1, num2, den2) {
        ...
        [0, 1].forEach(val => {
            ...
            tick.setAttribute('x1', x); tick.setAttribute('y1', svgHeight / 2 - 5);
            tick.setAttribute('x2', x); tick.setAttribute('y2', svgHeight / 2 + 5);
            tick.classList.add('tick-major'); numberLineSvg.appendChild(tick);
            const lbl = document.createElementNS('http://www.w3.org/2000/svg', 'text');
            lbl.setAttribute('x', x); lbl.setAttribute('y', svgHeight / 2 + 20);
            lbl.textContent = val; lbl.classList.add('label-axis'); numberLineSvg.appendChild(lbl);
        });
        
        let maxDenForTicks = den1;
        if (den2 > 0 && den2 > den1) maxDenForTicks = den2;
        if (maxDenForTicks > 1 && maxDenForTicks <= 20) {
            for (let i = 1; i < maxDenForTicks; i++) {
                const x = margin.left + (i / maxDenForTicks) * lineWidth;
                const tick = document.createElementNS('http://www.w3.org/2000/svg', 'line');
                tick.setAttribute('x1', x); tick.setAttribute('y1', svgHeight / 2 - 3);
                tick.setAttribute('x2', x); tick.setAttribute('y2', svgHeight / 2 + 3);
                tick.classList.add('tick-minor'); numberLineSvg.appendChild(tick);
            }
        }

        if (den1 > 0) {
            const val1 = num1 / den1;
            if (val1 >= 0 && val1 <= 1.0001) { // Allow slight floating point overflow for 1/1 etc.
                const x1 = margin.left + Math.min(val1, 1) * lineWidth; // Cap at 1
        ...
        if (den2 > 0 && num2 !== null && num2 >= 0) {
            ...
            if (val2 >= 0 && val2 <= 1.0001) {
                ...
                let yOffsetF2 = (den1 > 0 && Math.abs((num1/den1) - val2) < 0.1 && (num1/den1) <= 1.0001) ? svgHeight / 2 + 30 : svgHeight / 2 - 10;
                 if (den1 <= 0 || (num1/den1) > 1.0001) yOffsetF2 = svgHeight / 2 - 10; // If F1 not shown or out of bounds, F2 takes top spot
    ...
    function compareFractions(num1, den1, num2, den2) {
        if (den1 <= 0 || den2 <= 0 || num1 === null || num2 === null) {
    ...
    function validateFraction(numInput, denInput, messageElement, fractionName) {
        ...
        if (numStr === "" && denStr === "") {
            ...
            // F1 must have values (defaults are set)
        ...
        if (den > 0) numInput.max = den * 20; // Allow larger numerators, up to 20x denominator for flexibility
        else numInput.max = 100;

        messageElement.textContent = message;
        if (!isValid && isF2 && numStr === "" && denStr === "") { // Special case: F2 is optional and completely empty
             return { num: null, den: null, isValid: true, message: "" };
        }
        return { num: isNaN(num) ? (isF2 ? null : 0) : num, den: isNaN(den) ? (isF2 ? null : 1) : den, isValid, message };
    }
    ...
    function updateVisualizations() {
        ...
        if (f1.isValid && f2.isValid && f2.num !== null && f2.den !== null) {
